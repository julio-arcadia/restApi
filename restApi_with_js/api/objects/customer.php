...
class Customer {
   ...
   public function create() {
    ...
    public function update() {
        // Si el pass va a ser updated
        $password_set = !empty($this->password) ? ", password = :password" : "";
        
        // query para update
        $query = "UPDATE customer SET 	    	
                firstName=:firstName, lastName=:lastName, username=:username, country=:country,
                region=:region, city=:city, address=:address {$password_set}                
        ...
        $stmt->bindParam(':city', $this->city);
        $stmt->bindParam(':address', $this->address);
        // hacemos un hash al password antes de guardarlo
        if (!empty($this->password)) {
            $this->password = htmlspecialchars(strip_tags($this->password));
            $password_hash = password_hash($this->password, PASSWORD_BCRYPT);
            $stmt->bindParam(':password', $password_hash);
        }
        // enlazmos el id ID del record a editar
